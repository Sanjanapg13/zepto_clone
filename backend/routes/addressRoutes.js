...
router.post('/', authMiddleware, async (req, res) => {
  try {
    ...
    // Validate city and pin code match
    const validCities = ['Bangalore', 'Mumbai', 'Delhi'];
    if (!validCities.includes(city)) {
      console.log('Validation failed: Invalid city');
      return res.status(400).json({ success: false, message: 'Service only available in Bangalore, Mumbai, or Delhi' });
    }
    ...
    if (pinPrefix !== validPrefixes[city]) {
      console.log(`Validation failed: Invalid pin code for ${city}`);
      return res.status(400).json({ success: false, message: `Invalid pin code for ${city}. Pin codes must start with ${validPrefixes[city]}.` });
    }
