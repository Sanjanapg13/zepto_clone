...
const VendorDashboard = ({ user }) => {
  ...
  useEffect(() => {
    const fetchVendorProducts = async () => {
      try {
        const response = await fetch(`http://43.204.188.173:5000/api/products?vendorId=${user.id}`, {
          headers: {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await fetch('http://43.204.188.173:5000/api/orders/vendor', {
          headers: {
            'Authorization': `Bearer ${user.token}`,
          },
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://43.204.188.173:5000/api/products', {
        method: 'POST',
        headers: {
